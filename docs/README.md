...
Bộ tài liệu rút gọn cho mục tiêu demo local/testnet.
...
- `LOCAL_DEPLOY.md`: Cách chạy Hardhat local và deploy để frontend đọc dữ liệu.
...
- Phù hợp cho demo kỹ thuật trên local Hardhat và testnet (không phải bản mainnet production).
...
Deploy local: xem `LOCAL_DEPLOY.md`.
